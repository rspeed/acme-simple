...
This is a simple, auditable script that you can throw on your server to issue
...
key, I tried to make it as straightforward as possible (currently less than 200 lines).
...
acme-simple, to register an account for you and sign all following requests.
...
[JWK](https://tools.ietf.org/html/rfc7517) format. `acme-simple` is using the PEM
...
python acme_simple.py --account-key ./account.key --csr ./domain.csr --acme-dir /var/www/challenges/ > ./signed.crt
```
...
```sh
#!/usr/bin/sh
python /path/to/acme_simple.py --account-key /path/to/account.key --csr /path/to/domain.csr --acme-dir /var/www/challenges/ > /tmp/signed.crt || exit
wget -O - https://letsencrypt.org/certs/lets-encrypt-x3-cross-signed.pem > intermediate.pem
...
0 0 1 * * /path/to/renew_cert.sh 2>> /var/log/acme_simple.log
```
